...
# =========================================================
    ...
    # 아이폰 스타일 캘린더 디자인 유지 및 네이티브 버튼 스타일링 CSS
    st.markdown("""
    <style>
    ...
    for idx, w in enumerate(weekdays):
        ...
        head_cols[idx].markdown(f"<div style='text-align: center; font-weight: 500; font-size: 12px; padding: 6px 0 10px 0; color: {color};'>{w}</div>", unsafe_allow_html=True)

    # 캘린더 날짜 그리드 렌더링 (상단 탭과 동일한 네이티브 세션 상태 업데이트 방식 적용으로 깜빡임 원천 차단)
    for week in cal:
        cols = st.columns(7)
        for day_idx, day in enumerate(week):
            with cols[day_idx]:
                if day != 0:
                    ...
                    dot_color = "#ffffff" if is_selected else "#ff3b30"
                    dot_visibility = "visible" if has_log else "hidden"
                    st.markdown(f"""
                    <div style='text-align: center; margin-top: 1px;'>
                        <div style='width: 3px; height: 3px; background-color: {dot_color}; border-radius: 50%; margin: 0 auto; visibility: {dot_visibility};'>•</div>
                    </div>
                    """, unsafe_allow_html=True)
